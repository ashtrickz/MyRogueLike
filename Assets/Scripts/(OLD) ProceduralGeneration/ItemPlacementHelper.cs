...
namespace ProceduralGeneration
{
    ...
    {
        private Dictionary<PlacementType, HashSet<Vector2Int>> tileByType = new();

        private HashSet<Vector2Int> roomFloorNoCorridor = new();

        public ItemPlacementHelper(HashSet<Vector2Int> roomFloor, HashSet<Vector2Int> roomFloorNoCorridor)
        {
            Graph graph = new(roomFloor);
            this.roomFloorNoCorridor = roomFloorNoCorridor;

            foreach (var position in roomFloorNoCorridor)
            {
                int neighboursCount8Dir = graph.GetNeighbours8Direction(position).Count;
        ...
        {
            List<Vector2Int> positions = new() {originPosition};
            ...
            for (int row = minX; row < maxX; row++)
            {
                for (int col = minY; col < maxY; col++)
                {
                    if (col == 0 && row == 0)
                        continue;
                    Vector2Int newPosToCheck = new(originPosition.x + row, originPosition.y + col);
